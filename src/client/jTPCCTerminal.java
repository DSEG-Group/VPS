...
public class jTPCCTerminal implements jTPCCConfig, Runnable
{
	...
	private int transPior;//change 11.13
	...
	for(int i = 0; (i < numTransactions || numTransactions == -1) && !stopRunning; i++)
	{
	    ...
	    {
		parent.signalTerminalEndedTransaction(this.terminalName, transactionTypeName, transactionEnd - transactionStart, null, newOrder,transPior);//change 11.13
	    }
	    else
	    {
		parent.signalTerminalEndedTransaction(this.terminalName, transactionTypeName, transactionEnd - transactionStart, (skippedDeliveries == 0 ? "None" : "" + skippedDeliveries + " delivery(ies) skipped."), newOrder,transPior);//change 11.13
	    }
